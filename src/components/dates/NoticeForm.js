...
export default function NoticeForm({ initialChannels, initialDates, initialNotice }) {
  ...
  const [sendTime, setSendTime] = useState(initialNotice?.sendTime || config.notice.send_time);
  const [dayAfter, setDayAfter] = useState(initialNotice?.dayAfter || config.notice.after_day);
  const [dayBefore, setDayBefore] = useState(initialNotice?.dayBefore || config.notice.before_day);
